...
public interface UserDao extends JpaRepository<User, Long> {
    @Query(nativeQuery = true, value = "SELECT id FROM users WHERE mail=?1 and password=?2 and role=?3")
    Long findByEmailAndPassword(String mail, String password, String role);
